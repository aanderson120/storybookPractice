<script>
  // AspireSwitch.svelte
  // Makes toggle switches or yes/no buttons.
  //
  ...
  export let options = { true: "yes", false: "no" };
  ...
  function handleClick(event) {
    ...
    // value = checked === true ? options.true : options.false
    // After resetting the toggle, invoke the callback function, if there is one.
    if (clickFn) {
      clickFn();
    }
  }
...
{#if design == "inner"}
  <div class="s s--inner">
    ...
    >
      <span>{options.true}</span>
      <span>{options.false}</span>
    </button>
  </div>
